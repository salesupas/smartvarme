			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						if ( ! function_exists( 'unimicro_delivery_days_show' ) ) {
	function unimicro_delivery_days_show() : string {
		global $product;
		...
		$delivery_days          = ( $delivery_days && strtolower( $delivery_days ) !== 'null' ) ? (int) $delivery_days : 0;
		...
		if ( $current_stock <= 0 ) {
			return ( $delivery_days > 0 )
				? sprintf( '%d dagers leveringstid', $delivery_days )
				: '3 dagers leveringstid';
		}
...
if ( ! function_exists( 'custom_stock_display' ) ) {
	function custom_stock_display() : void {
		global $product;
		...
		if ( $qty > 0 ) {
			printf(
				'<p class="ast-stock-detail"><span class="stock in-stock">%s</span></p>',
				'På lager / Sending er om 1-2 dager'
			);
		} else {
			$delivery_msg = unimicro_delivery_days_show();
			if ( $delivery_msg ) {
				printf(
					'<p class="ast-stock-detail"><span class="stock in-stock">%s</span></p>',
					esc_html( $delivery_msg )
				);
			}
		}
	}
